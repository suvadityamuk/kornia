...
class ColorMap(ABC):
    ...
    def __init__(
        ...
        self.colors = keras.ops.image.resize(
            _list_color_to_tensor(base_colormap, dtype=self._dtype)[None, ...],
            num_colors,
            mode='linear',
        )[0, ...]
...
def apply_colormap(input_tensor, colormap: ColorMap):
    ...
    keys = keras.ops.arange(start=0, stop=len(colormap), dtype=input_tensor.dtype)  # (num_colors)

    index = keras.ops.digitize(input_tensor, keys)  # shape equals <input_tensor>: (B x H x W) or (H x W)

    output = colormap.colors[:, index]  # (3 x B x H x W) or (3 x H x W)

    if len(output.shape) == 4:
        output = keras.ops.transpose(output,axes=(1, 0, -2, -1))  # (B x 3 x H x W)
